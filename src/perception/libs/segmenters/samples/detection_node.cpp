...
void OnPointCloud(const sensor_msgs::PointCloud2ConstPtr& ros_pc2) {
    ...
    *cloud_nonground = *cloud_clusters[1];
    ...
    if (is_object_builder_open_) {
        ...
        /* START CUSTOM CODE -VV */
        if (use_roi_filter_) {
                ROS_INFO_STREAM("Trying Person/Car ROI Filtering\n");
                VolumetricModel vmCar;
                vmCar.model_type = CAR;
                vmCar.l_min = 20; vmCar.l_max = 50;
                vmCar.w_min = 20; vmCar.w_max = 50;
                vmCar.h_min = 20; vmCar.h_max = 50;

                VolumetricModel vmPer;
                vmPer.model_type = PEDESTRIAN;
                vmPer.l_min = 0.2; vmPer.l_max = 0.7;
                vmPer.w_min = 0.2; vmPer.w_max = 0.7;
                vmPer.h_min = 0.2; vmPer.h_max = 2.0;
                roi::humanCarModelFilter(objects,
                        vmPer,
                        vmCar,
                        &objects);
                ...
                for (int i = 0; i < objects.size(); i++) {
                        ...
                        else if (objects[i]->type == CAR) {
                                numCar++;
                        }
                }
                ROS_INFO_STREAM("Num Ped: " << numPerson << " Num Car: " << numCar << "\n");
        }
        /* END CUSTOM CODE */
